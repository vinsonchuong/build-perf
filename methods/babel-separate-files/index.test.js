...
test('compiling npm packages and serving individual files', async (t) => {
  const [middleware, buildTime] = await measure(async () => {
    return await build('big-dependency')
  })

  const server = await startServer({port: 10003}, compose(
    middleware,
  ...
  await navigate(tab, 'http://localhost:10003/')
  await findElement(tab, '#app-container', 'Hello World!')
